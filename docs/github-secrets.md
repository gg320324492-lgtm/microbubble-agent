...
## app 启动 lazy import (W67 第 40 步, 真治本)

主指挥决策**单次打破 0 production code 改动铁律**, 因为 qa-bench D5 gate CI 修复链 11 次尝试 (W67 第 29-39 步) 都卡在 uvicorn 启动 25+ min (实测 1510s), 逼近 1800s health budget。之前 11 次都是治标 (调 budget / 拆 build / GHA cache), 这次是真治本。

**真根因 (importtime profiling 定位)**: `app/services/file_service.py` 模块末尾 `file_service = FileService()` 单例, 其 `__init__` 直接调 `_ensure_bucket()` + `_set_public_read_policy()` — **两个同步阻塞 MinIO 网络请求**。import 链是 `app.main → app.agent.tools → drive_tools → drive_service → file_service`, 即**任何 import app.main 都会在模块加载阶段打 MinIO 网络**。本地 MinIO 立即拒签 (<1s), 但 CI/启动早期 MinIO 未就绪时 minio-py 带 backoff 反复重试 → import 卡 25+ min → uvicorn 迟迟不 ready。mimo SDK / Whisper / edge-tts / SenseVoice ASR 均**不**在 import 阶段做网络或加载模型, 排除嫌疑。

**修法 (lazy init, 非 lifespan 搬迁)**:
- `FileService.__init__` 不再碰网络; `client` 改成 `@property`, MinIO client 构造 + bucket/policy ensure 全部延迟到**首次真正用 MinIO** 时 (仅跑一次, `_bucket_ready` 置位防每请求重试)。
- `generic_chunked_upload_service.py` 的 `__init__` 里 `self._client = file_service.client` 同样改惰性 property, 避免其模块单例又触发一次。
- **为什么不搬到 lifespan**: uvicorn 在 lifespan startup 到达 `yield` 前**不接受任何请求**, 把 import 搬进 lifespan-before-yield 对 `/health` 早可用**零收益**且徒增破坏面。真治本是让 import 本身不打网络。

**效果**: `from app.main import app` 从 25+ min → **4.6s** (本地实测)。`/health` 与 `/auth/login` 在 uvicorn ready 后立即可服务, MinIO 功能不变 (首次上传/下载时 lazy ensure bucket + public-read policy)。

**关键铁律**:
1. **禁止在模块 import / 单例 `__init__` 阶段做网络 IO** (MinIO / HTTP / DB)。CLAUDE.md 方案 C 铁律 1 (跨 event loop) 的姊妹条: 网络调用必须惰性化到首次请求, 既保 loop 安全又保启动速度。
2. **import 失败必须 raise, 不 silent fail** — lifespan / import 阶段 crash 让 container exit, 比静默降级更安全。本次改动保持: MinIO 首用失败仍抛原异常, 只是时机从 import 挪到 request。
3. **改后必跑**: `bash scripts/check_typing_imports.sh` (142 文件 0 错误) + `python -c "from app.main import app"` (Docker 环境, 本地需 `pip install edge-tts`)。
4. **诊断工具**: `python -X importtime -c "import app.main" 2>log` → 按累计耗时排序找最慢 import; 若日志里出现 `S3Error` / `SignatureDoesNotMatch` 等运行时错误, 说明有模块在 import 阶段打网络。
