...
> 最后更新: **2026-06-24 sentence-transformers 5.6.0 升级（Phase 1+2 收官，Phase 3 跳过）+ v29 Qwen3 全量迁移 + v28 论文阅读器 8 phases 收官（commit `c8d4df3e` + `468c2b86` + 164 个 v28 step 109.x 打磨 commits）** — ① **ST 5.6.0 跨 3 大版本 0 破坏**（原 plan 担心 ST 3.x 破坏性变更，实测无影响）② **删 170 行 Qwen3 wrapper** → `qwen_embedder_legacy.py` (DEPRECATED 注释保留) + `embedding_service.py` 单 ST 路径重构 ③ **Qwen3 max_seq_length 2048 → 32768** (4x 上下文) ④ **ONNX 在 GPU 上慢 12-22x**（实测反优化，主动跳过）⑤ **qa-bench 50 题 38% → 42%**（反升 4%，超预期）⑥ **v29 三步全收官**：step 1 GPU 启用 + step 2 Qwen3 wrapper + alembic 030 双列 + 重算 350/351 + step 3 列原子切换 ⑦ **v28 8 phases 100% 收官** + article 9 字 bug 修复（深坑：INTERNAL_MARKER_RES 误删 `[PAGE:N]`）⑧ **CLAUDE.md 新增 5 大铁律**：清华源限速 / docker build env 污染 / ONNX 反优化 / ST 跨大版本 3 phase / Qwen3 native loading ⑨ 4 篇 PDF 37 张图 100% 核心不变量 + 内嵌图 confidence ≥ 0.85 阈值 + IO Hysteresis 防跳变 ⑩ **完整 plan + 实测**：[docs/upgrade-sentence-transformers-plan.md](docs/upgrade-sentence-transformers-plan.md)
