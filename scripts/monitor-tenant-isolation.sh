...
#   monitor-tenant-isolation.sh - 多租户数据隔离检测 (本脚本, W74 新增)
#
# 退出码: 0=正常 (全部 422), 1=异常 (跨租户 200), 2=执行错误
#
# 用法:
...
log "[1/4] 验证 TenantIsolationViolation 异常类"
...
log "  OK: TenantIsolationViolation (status=422) 已定义"

# 2. 验证 6 商业化表 alembic 083 索引 (W73 B-1 实施)
log "[2/4] 验证 alembic 083 多租户索引"
...
# 4. 跑跨租户 422 实战验证 (走 stress_tenant_isolation.py)
log "[4/4] 跑跨租户 422 实战压测 (100 并发 × 100 iter)"
