...
add_para('W3 收官：发现 #009 检索质量回归 bug (MicroBubbleAgent.chat_stream 缺 model 参数)', italic=True)
...
add_h3('3.1.3 #009 检索质量重做 + 用户深度思考开关（4 commits 收官）')
add_bullet('Phase 0.5 gate (Haiku judge + deterministic 改写 + re-retrieve, 7/14 R5/R6 benchmark 证伪后已删除)')
add_bullet('前端 useUiStore useDeepThinking + ToolContext kwarg 透传 + 检索质量相关 flag')
...
add_para('用户决策信号：当前 agentic_loop.py 包含 Phase 0/1/2 + 流式 abort + JSONB flag_modified + 4 域 fan-out + intent_classifier 集成，单文件 1123 行。')
...
add_h3('5.4.1 #009 检索质量离线评估决策')
add_para('当前检索质量监控字段 (retrieval_quality_score / retrieval_attempts) 仍保留在 agent_traces 表，但未跑离线重检索评估。')
...
add_bullet('#009 检索质量持续微调（字段写入 + 离线评估）')
