"""
v2: 用 flag_modified 强制写 transcript 修改
(否则 SQLAlchemy 不会 dirty track dict in-place 变化)
"""
import asyncio
import sys
import numpy as np
...
async def main():
    ...
    async with sf() as db:
        ...
        print(f"[2] loaded {len(enrolled)} enrolled", flush=True)
        ...
        print(f"[3] loaded audio: {len(pcm)/sr:.1f}s", flush=True)
        ...
        for j, i in enumerate(valid_idx):
            seg_embs[i] = embs[j]
        print(f"[5] embs: {sum(1 for e in seg_embs if e is not None)}", flush=True)
        ...
        for cid, (name, votes, total, ratio) in cluster_names.items():
            ...
            for i in range(len(transcript)):
                if labels_full[i] == cid and seg_embs[i] is not None:
                    ...
                    n_updated += 1
        print(f"[6] {n_updated} segments updated in memory", flush=True)
        
        # 关键: 用 flag_modified 标记 transcript 字段被修改
        ...
        await db.flush()
        await db.commit()
        print(f"✅ 写回 DB: {n_updated} segments, {len(m.speaker_mapping)} clusters")
