...
def main():
    ...
    out.append("- ✅ W3 真实发现 Self-RAG #009 回归 bug (生产 SSE 100% 失败)")
    out.append("- ✅ W3 prompts.py 4 域 5 段 checklist 强化")
    out.append("- ✅ W4 Self-RAG 3-tier 阈值分档 (0.8/0.6/0.4)")
    ...
    out.append("- W3 跑测 + 维度报告 + bug 修复: 40h (含 Self-RAG #009 回归)")
    ...
    out.append("- 高级能力 0% → **102 题 P 专项** 覆盖 (Self-RAG / fan-out / plan_step / 持久化 / abort / grounding)")
    out.append("- KB 入库: 半自动 → **全自动 + 5 道防线** (含灰度 + 7 天 rollback)")
    out.append("")
    out.append("### 收益")
    out.append("- 避免 1 次生产事故: W3 真实发现 Self-RAG #009 SSE 100% 失败 bug")
    ...
    out.append("| W3 | 跑测 + 维度 | 真实发现 Self-RAG #009 回归 + intent 标签 + 4 域 checklist | ✅ |")
    out.append("| W4 | 高级专项 | P 102 题 6 子类 + Self-RAG 3-tier 阈值分档 | ✅ |")
