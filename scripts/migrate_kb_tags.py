...
SCOPE_ALL = "all"  # P2-10 fix: 同时跑 auto + notes (合并 report 输出, apply 仍分开)
...
async def main() -> int:
    ...
    p.add_argument(
        "--scope",
        choices=[SCOPE_AUTO, SCOPE_NOTES, SCOPE_ALL],
        default=SCOPE_AUTO,
        help=(
            f"范围模式 (默认 {SCOPE_AUTO}, '{SCOPE_NOTES}' 笔记 category 模式, "
            f"'{SCOPE_ALL}' 同时跑两个 scope 合并 report)"
        ),
    )
    p.add_argument(
        "--confirm",
    ...
    try:
        # P2-10 fix: --scope all 时跑两次 (auto + notes), 合并 report 输出
        # apply 仍必须分开 (因为 NORMALIZED_TAG 替换规则 + delete_keywords 不同)
        scopes_to_run = [SCOPE_AUTO, SCOPE_NOTES] if args.scope == SCOPE_ALL else [args.scope]

        if args.scan:
            for s in scopes_to_run:
                log.info("[SCAN] 模式: 只查询不写库, scope=%s", s)
                report = await scan_kb(engine, scope=s)
                print_scan_table(report, args.limit)
            return 0

        # ── apply 流程 ──
        if args.scope == SCOPE_ALL:
            # SCOPE_ALL 不支持 apply (会破坏 report 与 apply 的一致性)
            log.error(
                "[APPLY] 不支持 --scope all (auto 和 notes 的 tag 替换规则不同, "
                "请分开跑: --scope auto_expansion --confirm && --scope notes_category --confirm"
            )
            return 1
