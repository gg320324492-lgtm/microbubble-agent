...
# §8 baseline 71 PASS + 7 SKIP 守恒
# ============================================================
section "§8 baseline 71 PASS + 7 SKIP 守恒"
...
echo ""
if [ "$FAIL" -eq 0 ]; then
    printf "%s✅ W68 第 7 批 15 commits + 3 hot-fix 部署验证全部通过%s\n" "${GREEN}${BOLD}" "${RESET}"
    echo ""
    echo "下一步:"
    ...
    echo ""
    echo "下一步:"
    echo "  - 按上方 FAIL 详情逐条修复 (优先: alembic 065 → hot-fix 真跑 → endpoint 鉴权)"
