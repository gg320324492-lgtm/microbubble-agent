...
背景:
- B-2 (alembic 066) 已建 knowledge_rejected + knowledge_pending_review 表
...
说明:
- 066 migration 已把这 3 列加在表上 (避免双迁移), 实际本迁移是个 no-op
- 保留本迁移文件以满足串单链 + 让 alembic head 067_push_subscriptions? 不对, 066 已是新 head
- 实际场景: B-3 与 B-2 在同 agent 实施 → 067 是占位"已完成", 仅更新 revision 字符串
...
- W68 第 3 批串单链纪律: down_revision 接 066_knowledge_rejected (B-2 后续)
- 067 = B-3 retry/suspend/health-check 收尾 (占位 no-op, 字段已在 066 加好)
"""
...
revision: str = "067_knowledge_rejected_retry"
down_revision: Union[str, None] = "066_knowledge_rejected"
branch_labels: Union[str, None] = None
depends_on: Union[str, None] = None


def upgrade() -> None:
    # 066 migration 已把 retry_count / next_retry_at / permanent_suspended 加在 knowledge_rejected 表上
    # (避免 067 重复 ALTER TABLE 报 column already exists)
    # 本 migration 仅作为 alembic 串单链占位, 真正的字段定义在 066
    #
    # 防御性 ALTER: 用 IF NOT EXISTS 兼容老库 (067 已存在但 066 还没跑过)
    op.execute(
