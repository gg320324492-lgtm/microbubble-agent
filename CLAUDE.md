...
13. **类 20.154 (新增, 永久铁律, W2 +N 服务器关机后 celery worker 未自启, 2026-08-13 实测)**: Docker Compose `restart: unless-stopped` **只响应 daemon 重启, 不响应容器自行 exit 0**. 服务器关机时 Docker 给所有容器发 SIGTERM, worker 优雅 Warm shutdown (exit code 0, `State.FinishedAt=2026-08-06T09:09:03`), Docker 把"正常退出"当成"用户主动停"**不自动拉起**. 实测 7 天后 `celery-worker-1` + `celery-meeting-worker-1` 仍 `Exited (0) 7 days ago`, 但 `celery-beat-1` + `app-1` + `db-1` + `redis-1` + `nginx-1` 都正常 Up —— 因为它们这次没正常 exit 或被 OOM/异常杀触发 restart. **修复**: `docker compose up -d celery-worker celery-meeting-worker` → 11s Up → `celery -A app.core.celery inspect ping` 2 nodes OK. **预防 (3 选项)**: ① `docker-compose.yml` 把 worker 的 `restart` 改成 `always` (注意: 真要停时用 `docker compose stop`, 不能 down) ② 加 healthcheck `celery inspect ping -t 5 || exit 1` (Compose 2.x 起 healthcheck 失败触发 restart) ③ `scripts/restart-recovery-after-gui-restart.sh` 加一步 `docker ps -a | grep celery-worker | grep Exited` → 有就 up -d. **会话启动纪律**: 任何会话启动必跑 `docker ps -a --format "table {{.Names}}\t{{.Status}}" | grep Exited` 排查, 看到 `celery-worker-1 Exited (0)` 一律 `docker compose up -d celery-worker celery-meeting-worker`. **不要信"服务器起来了一切都正常"** —— worker 退 1 步没起, 会议/提醒/驱动协作刷盘全卡. **关联类 20.143 自愈基础 (Winlogon EventID=7002 → docker restart)**, 类 20.154 是其盲点扩展.

14. **类 20.155 (新增, 永久铁律, W2 +N 服务器关机恢复后前端 401→429 循环, 2026-08-13 实测 + 2026-08-14 4 项修复实施)**: 服务器关机 7 天恢复后, 后端完全健康 (`/health` 200, alembic head `105_fix_drift` 守恒, celery worker 都已 up -d), **但前端浏览器一直 401 + refresh 风暴触发 429 Too Many Requests + WS 403 + redirecting to login 循环**. 根因链 3 层: ① **JWT token 失效**: 浏览器 localStorage 缓存了关机前旧 token (`exp=2026-08-06T18:01:10+08:00`), 服务端 secret 轮换或 in-memory cache 重启后清空 → 解析失败 401 ② **401 触发 refresh 风暴**: 前端 axios 拦截器对每个 401 自动调 `POST /api/v1/auth/refresh`, 并发 4+ 请求 (auth/me + dashboard/stats + notifications + members + WS 重连) 同时触发, 单 IP 瞬间 4+ 次 ③ **refresh 端点限流过严**: `/api/v1/auth/refresh` 走 `write: 30次/分` 限流, 401 风暴直接打爆 → 429 → refresh 也失败 → 前端清 token 跳 login → 新请求继续触发 refresh → 循环. WS 握手带旧 token 同样 403, 客户端 1-2s 重连又触发新一轮 401. **用户侧修复 (0 代码改动)**: 浏览器 DevTools → Application → Storage → **Clear site data** (清 localStorage + IndexedDB + SW) → 硬刷 `Ctrl+Shift+R` → 重新输入账号密码登录. **服务端预防 (4 项, 2026-08-14 实施完成)**: ① 前端 `web/src/utils/authRefresh.js` + `web/src/utils/request.js` + `web/src/utils/wsClient.js` 加 refresh 单飞 + 429 cooldown retry + wsClient.disableReconnect 联动 (vitest 6 case PASS) ② `/api/v1/auth/refresh` 限流从 `auth:20/分` 改为 `auth_refresh:60/分` 单独 tier (`app/core/rate_limit.py` 新增 `_AUTH_REFRESH_PATH` + dispatch 在 `_AUTH_SENSITIVE_PATHS` 之前), 实测 `X-RateLimit-Policy: auth_refresh` 头 ③ JWT secret 防御性加固 (`app/main.py` 启动日志 sha256[:8] + `app/config.py` `_validate_secret_key` validator, 实测 `.env` 已 bind mount 持久化, 0 production bug, 加保护性检查 + 文档 `docs/jwt-secret-management.md`) ④ `scripts/restart-recovery-after-gui-restart.sh` Step 8 + `docs/w100-meeting-pipeline-restart-2026-08-04.md` §7.5 + `scripts/auto-deploy.sh:323` 全部加前端 Clear site data 提示 + 根因解释. **诊断纪律**: 看到前端 401/429 互相切换 + redirecting to login 循环 + WS 403 → **不是后端问题, 是浏览器缓存的旧 token 问题**. 直接让用户浏览器 Clear site data, 不要去查后端日志 (后端健康, refresh 限流正常工作). **关联类 20.154 (同事件 celery worker) + 类 20.143 (自愈基础) + 类 20 全站限流 (auth:5/分, write:30/分)**. **未做留口**: 8 个 raw fetch 文件迁移 (留 follow-up PR) / JWT 双 key verify (留未来 PR).
