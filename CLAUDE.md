...
> **2026-07-03 当前任务链**：🆕 **v2 网盘 PR6-P16 external_userid case-insensitive uniqueness 收官** = **PR6-P15 模式复用价值体现 (4 个 identifier column 完整防御)** — **3 层防御**: ① alembic 056 `UNIQUE INDEX ON LOWER(external_userid)` 兜底真唯一 (revision `056_external_userid_ci` 22 chars 远 ≤32 限制) + ② service `_IDENTIFIER_COLUMNS` 白名单扩到 4 列 `{username, wechat_id, personal_wechat_id, external_userid}` + `_COLUMN_LABELS` 加 `"external_userid":"微信外部ID"` (wm 开头通常大写) + 旧 helper 完全不动 (PR6-P15 23 单测无需修改) + ③ API POST/PUT /members 双保险预检查 external_userid + **实际数据 0 冲突** (psql 验证 35 行 members 全部 external_userid 为空字符串, 迁移无需数据修复) + **业务动机**: `app/wechat/identity.py:41 IdentityResolver.resolve_by_external_userid()` 当前用精确匹配 `Member.external_userid == external_userid`, 防未来改 lower() 匹配时撞 map (与 PR6-P15 防 `resolve_by_wechat_id` 同模式) + 24/24 pytest PASS (6 helper + 5 labels + 3 create + 4 update + 3 alembic + 2 identity regression + 1 backward compat) + 92 passed, 9 skipped, 0 fail 合跑无回归 (PR6-P13 17 + PR6-P14 20 + PR6-P15 23 + PR6-P16 24 + drive_notification 8 = 92). **5 新铁律**: ① **PR6-P15 模式复用价值体现** — 加列只改 4 行 (`_IDENTIFIER_COLUMNS` 1 项 + `_COLUMN_LABELS` 1 项 + create/update 各 1 行 + API POST/PUT 各 1 行), 总计 ~15 行新代码 (相对 PR6-P13 的 ~75 行, 收益 5x); ② **测试断言 `>=` 而非 `==`** (PR6-P15 测试 `assert len(cols) == 3` 被 PR6-P16 打破 → 改 `len(cols) >= 3` 容忍未来扩展); ③ **IdentityResolver.resolve_multi_signal 是优先级链不是 `.resolve`** (`.resolve` 单查 wechat_userid, `.resolve_multi_signal` 才是 5 步优先级链: wechat_userid → external_userid → wechat_id → mobile → nickname); ④ **列名中文 label dict 比 if-else 硬编码灵活 5x** (加 4 列只改 dict 4 行, 加 1 列只改 dict 1 行); ⑤ **revision ID 长度踩坑沉淀** (PR6-P15 41 chars > VARCHAR(32) 教训, PR6-P16 命名 `056_external_userid_ci` 22 chars + 测试加 `assert len(mod.revision) <= 32` 防未来 PR). **5 文件**: `alembic/versions/056_external_userid_ci.py` (新, 35 行 CREATE UNIQUE INDEX `ix_members_external_userid_ci` ON LOWER(external_userid), 接 `055_personal_wechat_ci` 链) + `app/models/member.py` (改 3 行注释 PR6-P16) + `app/services/member_service.py` (`_IDENTIFIER_COLUMNS += "external_userid"` + `_COLUMN_LABELS["external_userid"]="微信外部ID"` + create/update 检查 external_userid) + `app/api/v1/member.py` (POST/PUT 都加 external_userid 检查) + `tests/test_member_external_userid_ci_unique.py` (新 280 行, 24 单测). **+ 1 个修复**: PR6-P15 测试 `test_identifier_columns_includes_personal_wechat_id` 硬编码 `len(cols) == 3` 被 PR6-P16 打破 → 改 `len(cols) >= 3` 容忍未来扩展.
> ---
