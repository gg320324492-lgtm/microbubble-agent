...
> ---
> **2026-07-02 早班 历史任务链**：🆕 **v2 PR6-P12 + Ollama 本地 LLM 部署收官** = **(a) cleanup_safety 守卫模式开关化** — `confirm_retention_param_auto` 统一入口 + settings 逗号分隔白名单 `_is_critical_task` 路由 + 3 task 顶部调用零差异 + 29/29 pytest PASS + 顺手修 pre-existing llm.py NameError + **(b) Ollama + Qwen3-8B 本地部署 10h 收官**（LLMClient backend dispatch 半成品→真正可用）。**Ollama 12 时间线**：vLLM 0.8.5 PyTorch 不支持 sm_120 失败 → AWQ 4-bit 反量化 (dequant_awq_to_fp16.py, AWQ gemm variant 8 4-bit per int32 LSB-first) 收 → FP16→GGUF 转换 56GB → F16→Q4_K_M 量化 OOM (28GB FP16 + 中间 buffer 需要 35-40GB RAM) → F16 56GB 直接跑装不进 RTX 5090 32GB VRAM → Ollama 0.31.1 + Docker Desktop 0.31.1 create API 全坏 (6 路径：Modelfile, files={gguf:}, hardlink, symlink cross-device, docker cp 56GB, 手动写 manifest 都返通用 error) → `POST /api/pull` 拉官方 qwen3:8b (5.2GB Q4_K_M) **✅ 5min 成功** → 验证 6 项 API (中文 chat ✓ 工具调用 ✓ OpenAI 兼容 ✓). **LLMClient.dispatch 收尾**：app/core/llm.py `complete()` 现在真分 3 个 backend (anthropic 行为不变 / openai_compat 抗 429 / ollama 本地), openai_response_to_anthropic_message() 修复 Pydantic ChatCompletion 模型访问 (`.get(...)` → 内部 `_get()` 兼容 dict + 对象) + tool_calls None 边界 + Anthropic Message 形状包装让 30+ caller 零迁移。**commits**: `d4fdab3f feat(llm): 2026-07-02 Ollama backend dispatch 收尾`. **6 端到端验证** (qwen3:8b via Ollama /v1)：① /api/tags 列出 5GB Q4_K_M + capabilities=['completion','tools','thinking'] ② 中文 chat "通义千问由通义实验室研发" (490ms) ③ 英文 generate "I am Qwen, developed by Alibaba Cloud" (387ms) ④ 工具调用 get_weather(city=Beijing) 200 OK 6.5s ⑤ OpenAI 兼容 chat "OK" (50ms) ⑥ OpenAI 兼容 + 工具 get_weather(city=Tokyo) finish_reason=tool_calls. **8 新铁律**：① Ollama 0.31.1 + Docker Desktop 0.31.1 = `create` API 全坏但 `pull` API 全好（永远走官方 pull） ② 14B 在 RTX 5090 32GB VRAM 装不下 F16 (28GB weights + 5GB KV = 33GB) 必须 Q4_K_M (~9GB) ③ F16→Q4_K_M 量化在 RAM 受限环境 OOM kill (需 35-40GB RAM) → 永远走官方 Q4_K_M 镜像 ④ Ollama 模型名 `^[a-zA-Z][a-zA-Z0-9_]*` 严格正则（基础名不能含 dash，tag 部分可以） ⑤ Ollama 不支持 cross-device symlink（Docker bind mount + 容器内 symlink 必失败） ⑥ Ollama `think=false` 加速 5-10×（`think=true` 推理中文 prompt "你好" 会输出 reasoning 后才给答案） ⑦ **LLMClient backend dispatch 是半成品**（`__init__` 选 client 但 `complete()` 旧版仍调 anthropic SDK，今回收 + Pydantic ChatCompletion 兼容 + tool_calls None 边界） ⑧ docker exec 看似 hang 但在跑（用 TaskNotification 等真正退出，不要靠输出判断）。**未来改进**：完整流式路径 (openai_compat + ollama stream/stream_raw) + 14b qwen3 模型测试 + mimo vs qwen3 35 题 benchmark 对比。**完整 memory**: `ollama-qwen3-deployment-2026-07-02.md`。
> ---
...
>
> **2026-07-02 早班收官 历史任务链**：v2 网盘 PR6-P2 前端 UI 3 件套收官（7 文件改动 / +928 -7 行）：audit 发现 PR6-P1 已实现 NotificationBell.vue（顶栏铃铛）+ ActivityFeedView.vue（活动流 + 路由 /drive/activity）+ useNotifications.js（Pinia store + WS + polling）— **唯一缺失 CommentThread + FileDetail 页面**。本轮交付：① **CommentThread.vue**（评论列表 + @ 自动 mention 解析 + 删除 + XSS escape + dark mode）② **FileDetailView.vue**（左右双栏布局：文件信息 + CommentThread + 在线预览/下载/收藏）③ 路由 `/drive/file/:id` 接入桌面端（移动端暂复用 DriveList 占位）④ FileCard/Grid/DriveView 默认单击跳详情 + Ctrl/Cmd 多选保持 ⑤ FilePreviewDialog 加"查看详情/评论"按钮（onGoDetail 跳详情页）。**4 新铁律**: ① 默认单击跳详情（符合主流网盘 UX, 主流 "Figma/Notion/Google Drive" 模式）② Ctrl/Cmd 多选保留（不让用户失去原有能力）③ comment content 渲染必须 escapeHtml + 仅对当前 mention 列表里的 username 高亮（不盲信任何用户输入）④ dark mode 跨组件覆盖必须非 scoped `<style>` 块（v60-v67 教训第 N 次强化）。**vitest**: **500/500 PASS**（excluded pre-existing useChatStream.ts regex 'l' flag 编译错误）。**端到端**: POST `/api/v1/drive/files/576/comments {"content":"@wangtianzhi ..."}` → 200 `mentions:[1]` `mentioned_user_ids:[1]` ✓。**stylelint 0 errors**。**清理**: DELETE FROM file_comments/mentions/activity_events WHERE file_id=576 → 1+1+4 条副作用全清.
