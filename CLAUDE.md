...
> ---
> **2026-07-02 历史任务链**：🆕 **v2 PR6-P12+ drive_cleanup_tasks 拆 service 函数收官** = **`clean_old_drive_files(db, cutoff_date)` service + thin Celery wrapper (与 chat_history_service 范式一致)** — drive_cleanup 物理删除逻辑 (235 行) 拆 2 层: `app/services/drive_cleanup_service.py` (NEW, 196 行) 接受 db + cutoff_date 返回 dict {deleted_files, deleted_folders, skipped_folders, minio_cleanup_failures}; `app/services/drive_cleanup_tasks.py` (refactor, 减 95 行) 保留 @celery_app.task 注册 + NullPool engine + 二次确认守卫 + asyncio.run + logger.warning 输出, 一行 `await clean_old_drive_files(db, cutoff)` 调 service. **关键选型**: 用 `backup_rows_to_json + 手动 DELETE WHERE safe_to_delete` (不是 `execute_backup_then_delete` 1 步模式), 因为 folder 表需 SELECT 检查"是否有子文件/子 folder" 决定是否删 (孤儿 folder 防御性). **MinIO 防御性**: `file_service.delete_file()` 抛异常 → `minio_cleanup_failures += 1` + logger.warning + DB 行仍硬删 (防 MinIO 故障阻塞 Celery beat). 59/59 pytest PASS (cleanup_backup 26 + cleanup_safety 29 + drive_cleanup_service 12 + 2 skipped). **5 新铁律**:① service/task 分层职责明确 (service 无 Celery, task 只做 Celery wrapper); ② `backup_rows_to_json` vs `execute_backup_then_delete` 按场景选型 (全删用 1 步模式, 条件删用 2 步模式); ③ MinIO 物理删除失败必须不阻塞 DB 硬删 (DB 一致性优先); ④ 孤儿 folder 防御性检查必须前置 (folder 表不能用 execute_backup_then_delete); ⑤ pytest async test 必须显式 `loop_scope="function"` 隔离 session loop (cleanup 系列合跑防 "no current event loop" 错). **commit**: `5e9d521c feat(drive): v2 PR6-P12+ drive_cleanup_tasks 拆出独立 service 函数`. **完整 memory**: `v2-drive-pr6-p12-drive-cleanup-service-2026-07-02.md`.
> ---
