...
> **2026-07-02 当前任务链**：🆕 **v2 网盘 PR6-P15 personal_wechat_id case-insensitive uniqueness 收官** = **PR6-P13/P14 通用化收官 (3 个 identifier column 完整防御)** — **3 层防御**: ① alembic 055 `UNIQUE INDEX ON LOWER(personal_wechat_id)` 兜底真唯一 (与 PR6-P13/054 镜像) + ② service `_IDENTIFIER_COLUMNS` 白名单扩到 3 列 `{username, wechat_id, personal_wechat_id}` + 新增 `_COLUMN_LABELS = {"username":"用户名","wechat_id":"企业微信号","personal_wechat_id":"个人微信号"}` 中文 label map (替代 if-else 硬编码) + 旧 helper 完全不动 (PR6-P14 20 单测无需修改) + ③ API POST/PUT /members 双保险预检查 personal_wechat_id + **实际数据 0 冲突** (psql 验证 35 行 members 全部 personal_wechat_id 为空字符串, 迁移无需数据修复) + 20/20 pytest PASS (7 generic helper + 3 create + 4 update + 1 backward compat + 3 alembic + 2 mention 回归) + 65 passed, 9 skipped, 0 fail 合跑无回归 (PR6-P13 17 + PR6-P14 20 + PR6-P15 20 + drive_notification 8 = 65). **5 新铁律**: ① **白名单是唯一扩展点** (未来加 personal_email / phone 只改 _IDENTIFIER_COLUMNS + _COLUMN_LABELS 2 处, helper 0 改); ② **`_COLUMN_LABELS` dict 中文 label 替代 if-else 硬编码** (3 行 vs 5+ 个 if-else 分支, 加列 O(1) vs O(N)); ③ **`app/wechat/identity.py:79 resolve_by_wechat_id` 当前精确匹配, 防未来 lower() 改写时撞 map** (提前兜底比事后清理成本低 10×); ④ PG 函数索引 NULL 不参与 UNIQUE, 多空字符串安全; ⑤ **`_assert_identifier_unique` 白名单 + 反射双保险** (列名仅接受白名单值, 反射 `getattr(Member, column_name)` 而非硬编码 if-else). **5 文件**: `alembic/versions/055_member_personal_wechat_id_ci_unique.py` (新, 50 行 `CREATE UNIQUE INDEX ix_members_personal_wechat_id_ci ON LOWER(personal_wechat_id)`, 旧版无 personal_wechat_id 索引无需 drop) + `app/models/member.py` (改 1 行注释 PR6-P15) + `app/services/member_service.py` (`_IDENTIFIER_COLUMNS += "personal_wechat_id"` + `_COLUMN_LABELS` dict 4 行 + create/update 检查 personal_wechat_id) + `app/api/v1/member.py` (POST/PUT 都加 personal_wechat_id 检查) + `tests/test_member_personal_wechat_id_ci_unique.py` (新 436 行, 20 单测). **commit**: `5bab3f15 feat(drive): v2 PR6-P15 personal_wechat_id case-insensitive uniqueness (alembic 055 + 3 层防御 + 20 单测 + .ollama SSH key 兜底)`. **附带 .gitignore 修复** (永久教训): `.ollama/` 整个目录 + `**/id_ed25519 / id_rsa / id_dsa / id_ecdsa / *.pem / *.key` 兜底规则 — `.ollama/id_ed25519` 是真实 OpenSSH 私钥 387 字节, 必须 ignore.
> ---
