...
> **2026-07-02 早班收官 当前任务链**：🆕 **v2 网盘 PR6-P3 移动端 review 3 件套收官**（4 文件改动 / +928 行）：PR6-P2 留下的"移动端暂用 DriveList 占位"空白本轮彻底填平：① **MobileCommentThread.vue** (NEW, 401 行) — 复用 desktop `useNotificationsStore` 零业务逻辑重复 + 紧凑布局 (单行 textarea + 32px 头像) + sticky 底部发布栏 + `ElMessageBox.confirm` 替代 `el-popconfirm` (mobile 体验更好) + 同样 XSS escape + @username 高亮 ② **MobileFileDetailView.vue** (NEW, 567 行) — 单列布局 + sticky 顶部 (返回+标题+操作) + `mfd-actions-primary` 三按钮 (预览/下载/收藏) + 长按底部 ... 触发 MobileActionSheet 7 操作 (预览/下载/收藏/重命名/移动/改可见性/删除) + 集成 MobileCommentThread + 复用 desktop `FilePreviewDialog` (el-dialog 已 isMobile 适配) ③ **router/index.js** (+5 行) — `mobile/MobileDriveView` 占位 → `mobile/MobileFileDetailView` 真实接入 ④ **MobileFileList.vue** (`onFileClick` 改写 + `useRouter` import) — 单击 = 跳详情页 (`router.push(\`/drive/file/${file.id}\`)`) 与桌面端 FileCard UX 镜像一致, 长按 600ms 仍由 LongPressWrapper 触发 ActionSheet 完整操作. **4 新铁律**: ① **移动端单击跳详情 + 长按操作菜单 = 主流 mobile 网盘 UX** (iOS Files / Android Files / Google Drive mobile / 百度网盘 mobile) — 不要反着做 (单击操作菜单会让用户每次点都误触) ② **XSS escape + @username regex 必须与 desktop 完全镜像** (`@([一-龥A-Za-z]{1,16})` + escapeHtml 5 字符先转义) — 前后端正则语义保持一致, 改一处必须改另一处 ③ **移动端删除必须 `ElMessageBox.confirm` 不要 `el-popconfirm`** — el-popconfirm 嵌入在小卡片按钮内 mobile 上 UI 错位 + 触摸区域太小易误触, ElMessageBox 居中弹出 modal 体验统一 ④ **复用 desktop store/composable 不要重写** (`useNotificationsStore.fetchComments/postComment/deleteComment`) — 业务逻辑零重复, 改一处两处都生效, 测试覆盖更广. **vitest**: **500/500 PASS** (exclude pre-existing useChatStream.ts regex 'l' flag). **stylelint**: 0 errors (修过 3 处 `#fff` → `var(--el-color-white)`, CLAUDE.md 永久规则). **端到端**: POST `/api/v1/drive/files/298/comments {"content":"@wangtianzhi ..."}` → 200 `comment.id:134 mentions:[1] mentioned_user_ids:[1]` ✓. **清理**: DELETE FROM file_comments/file_mentions/activity_events WHERE file_id=298 → 1+1+1 副作用全清. **未实现增量** (留给 PR6-P4+): @username autocomplete (mention 自动补全, 1 天); comment threading (parent_comment_id + MAX_DEPTH=3, 1.5 天); notification 5s dedup 窗口 (P2); Celery beat cleanup_old_mentions 30 天 + cleanup_old_events 90 天 (P3); drive_service.py 触发 notification_service (star/share/upload owner, P2); comment edit (service 已有, 缺 REST endpoint + 前端 edit dialog, P3); rich notification title/body (P3).
>
