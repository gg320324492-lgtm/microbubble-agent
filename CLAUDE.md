...
> **2026-07-12 当前任务链**: 🆕 **P0-#1 LLM_BACKEND=ollama 残留 → chat 全 Connection error** (1 commit, no code change 仅 `.env`+force-recreate) = **用户截图连续两条"你好"UI 显示 "⚠️ Connection error." (2.9s/2.6s)** → **根因 3 层**: ① `.env:86 LLM_BACKEND=ollama` 是 2026-07-02 Ollama 本地测试残留 (从未回滚到 openai_compat, CLAUDE.md 2026-07-02 决策 "生产保持 openai_compat, 8b 作 offline fallback"); ② 本地 PC Ollama 未在跑 (WinError 10061 connection refused; container 视角 host.docker.internal:11434 也连不上); ③ OpenAI SDK 1.x `APIConnectionError.__str__` 返 `'Connection error.'` (跟 anthropic SDK 0.39+ 一字不差, 仅看 e.message 区分不了) → curl 实测 SSE 3 个 error 事件 `AGENTIC_LOOP_ERROR/SYNTHESIZE_ERROR message="Connection error."` + reasoning `"intent classification failed: Connection error."`. **修复**: `.env` 改 `LLM_BACKEND=openai_compat` + `docker compose up -d --force-recreate app celery-worker` (⚠️ **restart 不重读 env_file 是大坑, 本会话再踩第 N 次**) → printenv 验证 `LLM_BACKEND=openai_compat` → curl SSE 验证 `text_delta: "你好！很高兴收到你的消息 🙋‍♂️\n\n我是 **小气**, 你的全能 AI 助手..."` ✅. **副 bug (本会话发现, 留后续 PR)**: intent_classifier.py:152-155 用 `for block in resp.content` 假设 Anthropic Message 形状, OpenAI 响应是 dict → `'dict' object has no attribute 'content'` → fallback 走 search_info+confidence=0, 不影响主流程但主路径也有同样隐患 (LLM 每次调 _complete_openai_compat 都 wrap 成 anthropic Message 应不影响, 但 intent_classifier 没走 wrapper, 设计漏洞). **3 新铁律**: ① `.env` 改完 `docker compose restart` **不重读 env_file** (CLAUDE.md 752 行铁律第 N 次强化), 必须 `up -d --force-recreate`; ② **OpenAI SDK 1.x + anthropic SDK 0.39+ `APIConnectionError.__str__` 都是 `'Connection error.'`**, 仅看 `e.message` 无法区分 backend, 必须看 traceback 或 printenv `LLM_BACKEND`; ③ **测试残留配置不留** — 任何 `LLM_BACKEND=*` / `OLLAMA_*` 临时测试改 `.env` 后 24h 内必须 git diff 检查 + revert (本次 11 天未回滚). **memory**: [`memory/llm-backend-ollama-residual-connection-error-2026-07-12.md`](./memory/llm-backend-ollama-residual-connection-error-2026-07-12.md). **commit 待 push**: 无代码改动, 跟踪 `.env` (gitignored) 改动需要 ✅ doc commit + memory + force-recreate 已生效 (生产已验证 SSE text_delta 正常流).
