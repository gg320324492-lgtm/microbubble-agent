...
> **2026-07-12 当前任务链**: 🆕 **P0-#1 + #1.5 双修收官 (3 commit 全 push)** = **P0-#1 `.env LLM_BACKEND=ollama 残留 → chat 全 Connection error`** (commit `20621c83`, 仅 `.env`+force-recreate) + **P0-#1.5 wrapper shape 不兼容 + mimo reasoning_content + intent_classifier max_tokens 3 重复合** (commit `9b908f50`, 4 文件 +263/-6) + **本 commit (memory 同步)**. **P0-#1.5 详细**: ① wrapper `openai_response_to_anthropic_message` 返 plain dict 但 docstring 写"调用方用 `.content/.stop_reason/.usage.input_tokens 属性访问"` — 设计漏洞, **12 caller** (intent_classifier / critic / self_rag / result_compressor / paper_layout / rag_evaluator / meeting_analysis 等) 全用 `resp.content` + `block.text` 属性访问 → `AttributeError: 'dict' object has no attribute 'content'` → intent/critic/compressor 全部永久 fallback; ② wrapper 不处理 mimo `reasoning_content` 字段 (OpenAI thinking 模型实际回答放 `reasoning_content` 而非 `content`); ③ intent_classifier `max_tokens=300` 不够 mimo reasoning (300 被 reasoning_content 占满 finish_reason=length → content 永远空). **修法 3 重复合**: ① 加 `_AnthropicMsgDict` (dict 子类 + `__getattr__`) + `_wrap_as_anthropic()` 递归 helper — `resp.content` 和 `resp["content"]` **双访问后向兼容 12 caller + 现有 dict 风格测试**; ② wrapper 加 `reasoning_content` → `{type:thinking, thinking:...}` Anthropic SDK 标准 block, 让 `extract_text_from_response` 等工具自动生效; ③ intent_classifier `max_tokens=300→2048` + fallback 到 thinking block 提取. **端到端**: 14/14 测试 PASS (Case 13 wrapper attr+dict 双访问 + Case 14 reasoning_content→thinking block) + docker compose up -d --force-recreate + curl SSE 60s 验证 `intent_detected reasoning='用户询问 dutonghe是谁, 属于查找人员信息, 因此归类为search_info' + label 置信度 95%` (vs 旧 0% fallback). **5 新铁律**: ① **wrapper shape 与 caller 期望必须对齐** — 用 `_AnthropicMsgDict` 字典子类 + `__getattr__` 双访问,**后向兼容 12 caller + 现有测试同时生效**; ② **OpenAI thinking 模型的 `reasoning_content` 必须 wrap 为 thinking block** — 否则 thinking-only 响应丢内容,纯文本 caller 全瞎; ③ **mimo-v2.5 thinking 模型 max_tokens 至少 2048** — 300 被 reasoning 占满 finish_reason=length → content 永远空; ④ **`thinking={"type":"disabled"}` 字段在 openai_compat 路径 `_complete_openai_compat` 不传递** — Anthropic 专属字段实际靠 max_tokens 兜底; ⑤ **`for block in resp.content` 是 anthropic backend 假设** — backend dispatch 后必须 wrapper shape 同步, 12 处 caller 全靠 wrapper 改一次全修. **memory**: [`memory/anthropic-msg-dict-wrapper-mimo-reasoning-content-2026-07-12.md`](./memory/anthropic-msg-dict-wrapper-mimo-reasoning-content-2026-07-12.md). **commit hashes**: `20621c83` (.env doc) → `9b908f50` (3 重复合修复) → 当前 (memory).
