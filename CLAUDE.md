...
> **2026-07-02 当前任务链**：🆕 **v2 PR6-P11 Celery retention 二次确认守卫收官**（5 文件 + 14 单测 / +213 行）= **3 个 Celery cleanup task (chat_history/drive_cleanup/file_mention) 加 retention_days 二次确认守卫 + 新增 app/services/cleanup_safety.py 模块**。继 PR6-P9 误传 `retention_days=0` 删 31 条 / PR6-P10 补 backup 机制 后的**第二道防线**：retention != settings 默认值时, 延迟 0.5s (`RETENTION_OVERRIDE_CONFIRM_DELAY_SEC`) + logger.warning 二段打印 (检测到 + 二次确认通过), 让人手能在 0.5s 内 Ctrl+C 取消。**双重 API**: `confirm_retention_param` (延迟+warn+proceed, 用户友好, 3 task 默认走这个) + `confirm_retention_param_or_skip` (严格模式, 非默认就拒绝, 留给未来 critical 场景)。**首次集成测试踩坑 (教训沉淀)**: `test_cleanup_soft_deleted_sessions_task_triggers_delay_on_retention_zero` 之前没真 mock service, 守卫 proceed=True 后 task 真跑 cleanup → 真 DELETE 了 4 条 chat_sessions。已用 PR6-P10 `restore_from_backup.py` 恢复, 测试改用 `_make_async_return(0)` mock service 返 0 行, 守住"测试只验证守卫被触发, 不真删数据"。**5 新铁律 (永久沉淀)**:① Celery retention 类参数必须 `confirm_retention_param` 守卫 (delay + warn) — 3 task 顶部统一 import ② 默认值 == settings.X_RETENTION_DAYS 时不触发守卫, 周期性 .delay() 调用永远走 None 路径不延迟 ③ 延迟秒数从 settings 读, 紧急场景可设 0 关闭 (真要快速跑非默认) ④ **测试时必须 mock service 函数返 0**, 守卫 proceed=True 后面是真 destructive cleanup ⑤ 严格版 `confirm_retention_param_or_skip` 留给 critical 场景 (Sentry 监控/严禁漂移), 默认用 confirm_retention_param 友好版。**端到端验证**: pytest 14/14 PASS + 3 task 集成测试模拟 retention=0 误传, 守卫 delay + warn 触发成功, 0 真 DELETE。
> ---
