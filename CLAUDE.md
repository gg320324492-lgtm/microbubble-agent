...
> **2026-07-14 当前任务链**: 🆕 **#009 Self-RAG R3/R4 benchmark 推翻 7/9 audit "7/30 删除" 建议 (5 文件改动 + 1 memory)** = **100 题 × 2 round ollama balanced (R3 Self-RAG ON vs R4 Self-RAG OFF)** 实测 pass rate **99/100 vs 100/100** (1 个 429 噪声) + Retrieval gate **0/100 触发** (vs 7/01 R2 anthropic 15/100). **核心推翻结论**: 7/9 audit 建议"7/30 截止时删除 Self-RAG flag" 因 7/13 commit `c2648120 feat(three-mode)` 集成后前提失效 — Self-RAG 已被吸收进 `ctx.thinking_config.self_rag_enabled` + `agentic_loop.py` 5 处真分支, **不再是 optional flag 而是 deep mode 防幻觉的最后一道防线** (commit message: "deep 有幻觉风险"). **3 文件改动**: ① `scripts/qa_bench_smoke.py` 加 `--thinking-mode` / `--use-self-rag` flags 透传 ChatRequest + 修 429 退避 30s + 修 < 10 题写盘 bug; ② `scripts/compare_self_rag_rounds.py` (新, 130 行) 4 轮对比 + delta + per-category + retrieval 触发统计; ③ `tests/qa-bench/results/self-rag-benchmark/REPORT.md` (替换 7/01 2 轮 REPORT 为 4 轮对比) + R3/R4 results.json. **2 新数据**: R3 `tests/qa-bench/results/self-rag-benchmark/round3-balanced-selfrag-on-1784030859/results.json` + R4 `tests/qa-bench/results/self-rag-benchmark/round4-balanced-selfrag-off-1784032275/results.json`. **新承诺**: Self-RAG 重新设 30 天观察期, 截止 **2026-08-14** (原 7/30 截止作废). **下一步 (待跑)**: R5/R6 deep mode benchmark (R5: deep + SelfRAG ON vs R6: deep + SelfRAG OFF, 100 题 × 2 round, ~80min). **5 新铁律** (详见 [`memory/self-rag-three-mode-r3r4-benchmark-2026-07-14.md`](./memory/self-rag-three-mode-r3r4-benchmark-2026-07-14.md)): ① **Ollama 性能漂移 4-5× (first-20 vs last-20)** — benchmark 必须报告 first/last 切片, avg 不可信, 只看 pass rate + per-category; ② **Self-RAG gate 触发率是新 Self-RAG 行为的唯一可信指标** — pass rate 不能区分 ON/OFF 差异, 必须直接看 retrieval_assessment event 数量; ③ **同 backend 内 ON vs OFF 对比是 gold standard** — 跨 backend (anthropic vs ollama) latency 差异 > 10× 是 noise; ④ **429 是 SSE tier 限流噪声, 不是 Self-RAG 问题** — smoke runner 加 30s 退避重试, benchmark 报告 ERR 时区分 429 vs 真质量问题; ⑤ **7/9 audit 建议会因新 commit 而过期** — 任何"30 天后删 XXX"的承诺必须在新功能集成时重新评估. **#009 历史回放**: 7/01 R1 (ant OFF) 100% / R2 (ant ON) 98% (-2%) / R3 (ollama bal ON) 99% (-1% 噪声) / R4 (ollama bal OFF) 100% — 7/13 commit 改造后 R3 数据不再支撑"删除"结论.
