...
<template>
  <section
    class="ozone-twin-panel"
    :data-status="device?.status ?? 'empty'"
    :aria-label="ariaLabel"
  >
    <header class="ozone-twin-panel__head">
      <span class="ozone-twin-panel__label">臭氧发生器孪生</span>
      <span class="ozone-twin-panel__status" :data-online="isOnline ? 'true' : 'false'">{{ statusLabel }}</span>
    </header>
...
.ozone-twin-panel__status[data-online='false'] {
  background: rgba(148, 163, 184, 0.18);
  color: var(--research-scada-muted, #94a3b8);
}
.ozone-twin-panel__name {
  font-size: 16px;
  font-weight: 600;
  color: var(--research-scada-text, #d6e4ee);
  margin-bottom: 12px;
}
.ozone-twin-panel__row {
  display: flex;
  justify-content: space-between;
  padding: 6px 0;
  border-top: 1px solid rgba(49, 67, 71, 0.5);
}
.ozone-twin-panel__metric-label {
  font-size: 12px;
  color: var(--research-scada-muted, #94a3b8);
}
.ozone-twin-panel__metric-value {
  font-size: 13px;
  font-weight: 600;
  color: var(--research-scada-text, #d6e4ee);
}
.ozone-twin-panel__empty {
  text-align: center;
  padding: 24px;
  color: var(--research-scada-muted, #94a3b8);
  font-size: 13px;
