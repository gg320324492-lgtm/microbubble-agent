...
describe('Phase 8-I3-A agent center', () => {
  ...
  it('renders real event timeline', () => { expect(c).toContain('ResearchTimeline') })
  it('maps timeline entries from agent events', () => { expect(c).toContain('agentStore.events.map') })
  it('has result cards', () => { expect(c).toContain('研究设计结果') })
  it('has observable AI research team', () => { expect(c).toContain('AI 研究团队') })
  it('uses AgentWorkspaceCard', () => { expect(c).toContain('AgentWorkspaceCard') })
  it('maps agents only from matching event text', () => { expect(c).toContain('item.label.includes(role.name)') })
  it('uses ToolExecutionPanel for actual tool records', () => { expect(c).toContain('ToolExecutionPanel') })
  it('has result card content', () => { expect(c).toContain('科研问题') })
  it('has real evidence panel', () => { expect(c).toContain('EvidencePanel') })
  it('maps tool calls from real messages', () => { expect(c).toContain('agentStore.messages.flatMap') })
  ...
  it('does not use workflowStore', () => { expect(c).not.toContain('useWorkflowStore') })
...
describe('Phase 8-I3-A extended coverage', () => {
  ...
  describe('very last 123 tests', () => {
    ...
    it('AC3 timeline derives from agent events', () => { const c = readFileSync(resolve(rendererRoot, 'pages/research/AgentCenter.vue'), 'utf8'); expect(c).toContain('const events = computed') })
    ...
    it('AC5 observable team', () => { const c = readFileSync(resolve(rendererRoot, 'pages/research/AgentCenter.vue'), 'utf8'); expect(c).toContain('AI 研究团队') })
    it('AC6 timeline panel', () => { const c = readFileSync(resolve(rendererRoot, 'pages/research/AgentCenter.vue'), 'utf8'); expect(c).toContain('<ResearchTimeline :items="events"') })
    it('AC7 tool execution panel', () => { const c = readFileSync(resolve(rendererRoot, 'pages/research/AgentCenter.vue'), 'utf8'); expect(c).toContain('<ToolExecutionPanel :executions="toolExecutions"') })
    ...
    it('AC11 literature agent', () => { const c = readFileSync(resolve(rendererRoot, 'pages/research/AgentCenter.vue'), 'utf8'); expect(c).toContain("name: '文献智能体'") })
    ...
    it('AC13 reviewer agent', () => { const c = readFileSync(resolve(rendererRoot, 'pages/research/AgentCenter.vue'), 'utf8'); expect(c).toContain("name: '审稿智能体'") })
    it('AC14 does not infer agent roles from event type', () => { const c = readFileSync(resolve(rendererRoot, 'pages/research/AgentCenter.vue'), 'utf8'); expect(c).not.toContain('event.type ===') })
    it('AC15 tool records come from real message tool calls', () => { const c = readFileSync(resolve(rendererRoot, 'pages/research/AgentCenter.vue'), 'utf8'); expect(c).toContain('(message.toolCalls ?? [])') })
