...
test.describe('Drive v2 PR6/PR8 integration — iPhone 14 Pro', () => {
  ...
  test('B: 文件 tab -> FAB -> ActionSheet -> upload succeeds', async ({ page, request }) => {
    ...
    if (mobile) {
      ...
      const sheet = page.locator('.sheet-overlay:visible, .sheet-panel:visible').first()
      await expect(sheet, 'FAB must open an upload ActionSheet').toBeVisible()
      await expect(sheet.locator('.action-item')).toHaveCount(3)
      const sheetText = await sheet.textContent()
      expect(sheetText).toMatch(/drive|入网盘/i)
      console.log('[B] mobile FAB opened the three-action upload sheet')
    } else {
  ...
  test('C: star a file -> 收藏 tab contains the starred file', async ({ page, request }) => {
    ...
    await page.goto(`${BASE_URL}/drive`, { waitUntil: 'domcontentloaded', timeout: 20_000 })
    await page.waitForSelector('.mobile-drive-view, .desktop-drive-view, .drive-page', { timeout: 20_000 })
    if (await isMobileDrive(page)) {
      await mobileTab(page, '收藏').click()
      await page.waitForTimeout(700)
      await expect(mobileTab(page, '收藏')).toHaveAttribute('aria-selected', 'true')
  ...
  test('D: 最近 tab shows the newest upload', async ({ page, request }) => {
    ...
    if (await isMobileDrive(page)) {
      await mobileTab(page, '最近').click()
      await page.waitForTimeout(700)
      await expect(mobileTab(page, '最近')).toHaveAttribute('aria-selected', 'true')
  ...
  test('E: 团队 tab shows a team-root upload', async ({ page, request }) => {
    ...
    if (await isMobileDrive(page)) {
      await mobileTab(page, '团队').click()
      await page.waitForTimeout(700)
      await expect(mobileTab(page, '团队')).toHaveAttribute('aria-selected', 'true')
      expect(await waitForFileName(page, uploaded.fileName)).toBe(true)
    } else {
