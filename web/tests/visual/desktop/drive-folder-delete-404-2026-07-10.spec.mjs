...
test.describe('drive-folder-delete-404-2026-07-10: AppException 格式 + 错误消息提取', () => {
  ...
  test('C: FolderServiceError 状态码映射（400/403/404 → Validation/Forbidden/NotFound）', async ({ page, context }) => {
    ...
    //   - 403: update folder not owner (FolderServiceError status_code=403)
    //
    // 用 xiaoqi_testbot 真实 folder 测试 (从 tree API 拿 ID)
    ...
    // 触发 delete (folder 28 在另一用户 alice_team_folder 下，xiaoqi_testbot delete 应返 404 + AppException)
