/**
 * useChunkedUploaderCore.js — W84 B-2 P1-1 通用 chunked upload 核心 (Step 1 兼容层)
 *
 * 派工依据 (W84 第 1 批 B-2):
 * - W82 A-2 Survey 3 §6 P1: chunked upload 3+ 套并存 (useChunkedUploader / useDriveChunkedUpload / useMobileUploadQueue)
 * - W84 B-2 P1-1 派工: 收敛 3 套, 分步走
 * - W82/W83 B-2 拦截铁律 (#16/#17): Step 1 仅建 useChunkedUploaderCore.js 兼容层, useChunkedUploader.js 改 thin-shell 委派
 * - W85 后续 batch 再删 useDriveChunkedUpload.js / useMobileUploadQueue.ts (Step 2)
 *
 * 设计:
 * - 抽出 useChunkedUploader.js 通用核心 API (uploadOne / uploadAll / enqueue / drainQueue)
 * - 单例会话音频 chunk 上传 — meetingId 维度, 内存 + IDB 混合
 * - 4xx 立即抛错 (业务错不重试); 5xx/网络错指数退避 (1s/2s/4s/8s/16s, max 30s)
 * - online 事件自动恢复 (内存队列 + IDB 双层扫描兜底)
 * - 与 useNetworkStatus 联动: 成功任意片 → markReachable; 5 次网络错耗尽 → markUnreachable
 *
 * 调用方 (保持兼容):
 *   import { useChunkedUploader, uploadOne, uploadAll, enqueue, drainQueue } from '@/composables/useChunkedUploader'
 *   // useChunkedUploader.js 仍导出, 委派到本 core
 *
 * 注意事项:
 * - module-level 单例 (isUploading / uploadQueue / onlineListenersAttached) 必须在 core 内维护, 不能委派到 useChunkedUploader.js
 * - 单元测试若要 reset 状态, 调用 _resetCoreForTests() (仅测试环境, 生产无副作用)
 */
...
export async function uploadOne(meetingId, index, blob, opts = {}) {
  ...
  let lastErr = null
  let sawNetworkError = false  // 追踪是否所有重试都是网络类错误
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      markReachable()  // 任何一片成功 → 立刻告诉网络层"通的"
      return  // 成功
    } catch (err) {
      lastErr = err
  ...
  try {
    for (const chunk of chunks) {
      try {
        ...
        console.warn(`[uploaderCore] chunk ${chunk.chunk_index} 失败:`, err?.message)
        failed++
...
export function enqueue(meetingId, chunks) {
  ...
  drainQueue(meetingId).catch((err) => {
    console.warn('[uploaderCore] drainQueue 失败:', err)
  })
}
 ...
 * 排空某会议的队列
 */
export async function drainQueue(meetingId) {
...
function ensureOnlineListener() {
  ...
  const onOnline = async () => {
    console.log('[uploaderCore] 检测到 online 事件, 扫描所有待传 chunks')
 ...
 */
export function useChunkedUploaderCore() {
  return {
...
export function _getCoreState() {
  return {
    isUploading,
    uploadQueue,
    onlineListenersAttached,
  }
}

/**
 * 仅测试环境 reset (单例状态不会自然清理)
 * W84 B-2 兼容层: 保留为 hidden API, 不暴露给生产调用方
 */
export function _resetCoreForTests() {
  isUploading.value = false
  uploadQueue.clear()
  // online listener 单例保留 (避免重复注册)
}

export default useChunkedUploaderCore
