 ...
 *
 * 验证明暗双主题 (light/dark) 下, 所有关键 token 都能解析到具体值。
...
describe('CSS variables: 明暗双主题 token 解析', () => {
  let variablesCSS
  ...
  THEME_MATRIX.forEach(({ theme }) => {
    it(`${theme}: 12 关键 token 都能解析`, () => {
      ...
      CRITICAL_TOKENS.forEach((token) => {
        ...
        expect(value, `token ${token} 在 ${theme} 下为空`).not.toBe('')
        // 颜色不能是 "rgb(0, 0, 0)" 默认 (代表未解析)
        // 注: dark 模式下文本色是 #e8eaed, 不会是纯黑, 这里只检查 "有值"
        expect(value, `token ${token} 在 ${theme} 下解析到 "initial"`).not.toBe('initial')
        expect(value, `token ${token} 在 ${theme} 下解析到 "currentColor"`).not.toBe('currentColor')
      })
      ...
      // 6. 验证暖橙品牌色 primary (v77 P2.6 fix 守恒: EP token 映射在 [data-theme] attribute block)
      if (theme === 'light') {
        ...
        expect(primary, `light primary 应含 255 和 122`).toMatch(/(255|122|7a)/i)
      }
  ...
  it('无主题属性时 token 仍可解析 (默认 light)', () => {
    // 不设 data-theme, 应使用 :root 默认值
    const style = document.createElement('style')
    style.setAttribute('data-v74-test', 'true')
    style.textContent = variablesCSS
    document.head.appendChild(style)
...
describe('CSS variable 守卫: 关键 token 不可缺失', () => {
  // v74 沉淀: 防止未来 refactor 删 token 导致主题全黑屏
  let variablesCSS
