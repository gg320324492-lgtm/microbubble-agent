...
# ============================================================================
# v28: 论文图片专项结构化分析 prompt
# ============================================================================
# 目标: 解决 mimo-v2.5 视觉模型把 Elsevier logo / 期刊封面误判为正文图
# 策略:
#   1. 类型判断优先级最高 - 先识别 logo/cover/publisher, 再识别 figureNo
#   2. Few-shot examples - 3 张对照图 (logo / 封面 / 真实图)
#   3. 强制 JSON markdown 包裹
#   4. 详细判别特征
# ============================================================================


PROMPT_FIGURE_ANALYZE = """你是学术论文图片分析专家。

# 判断顺序（必须按此优先级）

## 第一步: 先判断图片是否为非正文图 (isPublisherImage)

必须先判断! 若 isPublisherImage=true, 直接输出对应结果, 不再识别 figureNo.

判断为 isPublisherImage=true 的类型:

[类型 1: Elsevier / Springer / Wiley / ACS / Nature / Science 出版商 logo]
- 特征: 整张图主要是单个出版商名 ("ELSEVIER", "Springer", "Wiley")
- 可能含 logo 图标 (如 "ELSEVIER" 单独出现的页面)
- 通常无其他文字内容
- 例: 仅含 "ELSEVIER" 字样, 或仅含 "Springer" 标志

[类型 2: 期刊封面/期刊首页 (journal cover / title page)]
- 特征: 含 "Journal of Xxx" / "JOURNAL OF HAZARDOUS MATERIALS"
- 通常含 "Volume X" / "Issue Y" / 年份 / DOI / ISSN
- 通常是论文第 1 页上半部分
- 例: "ELSEVIER \n JOURNAL OF HAZARDOUS MATERIALS \n Environmental Contamination..."

[类型 3: 版权页/出版信息页]
- 特征: 含 "©" / "Copyright" / "All rights reserved" / "Published by"
- 含 "ISSN" / "ISBN" / "DOI.org/..."
- 整页都是出版信息

[类型 4: Graphical Abstract 摘要图]
- 特征: 通常是 page 1 上半部分
- 含 "Graphical Abstract" 字样
- figureNo 应该 = null (不算正文 Fig. 1)

[类型 5: Supplementary Cover / TOC]
- 特征: 含 "Supplementary Information" / "Table of Contents"

## 第二步: 识别正文图 (isCoreFigure=true)

只有非出版商图, 才识别 figureNo:

[图号识别]
- 主文图: "Fig. 1" / "Fig. 2" / "Figure 3" / "Fig. 5d"
- 主文表: "Table 1" / "Table 2"
- 主文示意图: "Scheme 1" / "Scheme 2"
- 补充材料 (不算正文): "Fig. S1" / "Fig. S2" / "Figs. S3-S4" → isSupportingFigure=true

[典型特征]
- 图中通常含化学式 (O3, H2O2, CO2 等)
- 含坐标轴标签 (x/y axis)
- 含数据点 / 柱状图 / 折线 / 散点
- 含 caption (如 "Fig. 2. Effect of X on Y.")

# 输出格式 (严格按此 JSON)

\`\`\`json
{
  "figureNo": "Fig. 2 | Fig. S2 | null",
  "figureType": "experimental_setup | chart | scheme | mechanism | table | cover | logo | publisher",
  "semanticTitle": "一句话简短描述图的主题",
  "visualSummary": "详细描述图的内容 (100-300字)",
  "sectionHint": "图最可能属于的章节标题或主题关键词",
  "isCoreFigure": true,
...
## Example 1 (Elsevier logo)
图片内容: 仅含 "ELSEVIER" 字样 + 一个简单的 logo 图标
输出:
\`\`\`json
{
  "figureNo": null,
...
## Example 2 (期刊封面)
图片内容: "ELSEVIER \n JOURNAL OF HAZARDOUS MATERIALS \n Environmental Contamination, Risk Assessment, Impact and Management"
输出:
\`\`\`json
{
  "figureNo": null,
  ...
  "visualSummary": "Elsevier 旗下 Journal of Hazardous Materials 期刊首页, 含期刊名和出版信息",
  "sectionHint": null,
  "isCoreFigure": false,
  "isPublisherImage": true,
  "isSupportingFigure": false,
...
## Example 3 (正文图 Fig. 1 热力图)
图片内容: 含 "Liquid Phase \n O3-MNBs \n H2O2 \n OH radicals" 等化学式 + 颜色热力图 + "Toluene Conversion (%)" 坐标轴
输出:
\`\`\`json
{
  "figureNo": "Fig. 1",
  "figureType": "chart",
  "semanticTitle": "Effects of O3 and H2O2 on toluene conversion",
  "visualSummary": "热力图展示 O3 浓度 (0-400 ppm) 和 H2O2 浓度 (0-100 mM) 组合下甲苯的转化率",
  "sectionHint": "Effect of O3 and H2O2 on toluene conversion",
  "isCoreFigure": true,
  "isPublisherImage": false,
  "isSupportingFigure": false,
  "confidence": 0.92
...
## Example 4 (补充材料 Fig. S2)
图片内容: 含 "Fig. S2" 字样 + 实验数据图
输出:
\`\`\`json
{
  "figureNo": "Fig. S2",
...
1. **必须先判断 isPublisherImage** - 这是判断 isCoreFigure 的前提
2. 严格 JSON 输出 (用 \`\`\`json 包裹), 不要任何额外解释文字
3. 没识别到的字段填 null, 不要编造
4. figureType 必填
5. confidence 反映你对判断的把握 (logo/封面应 ≥ 0.9)
"""
...
class OCRService:
    ...
    async def extract_figure_structured(
        self,
        image_bytes: bytes,
        mime_type: str = "image/png",
    ) -> dict:
        """v28: 论文图片结构化分析（识别 figureNo / figureType / isPublisherImage 等）

        返回 dict:
            {
                "figureNo": "Fig. 2 | Fig. S2 | null",
                "figureType": "experimental_setup | chart | scheme | mechanism | table | cover | logo | publisher | figure",
                "semanticTitle": str | None,
                "visualSummary": str | None,
                "sectionHint": str | None,
                "isCoreFigure": bool,
                "isPublisherImage": bool,
        ...
        try:
            ...
            logger.error(f"extract_figure_structured 失败: {e}")
            return {
                "figureNo": None,
                "figureType": "figure",
                "semanticTitle": None,
        ...
        # 解析 JSON（容忍 ```json ... ``` 包裹）
        return _parse_figure_structured_response(result_text)
...
def _parse_figure_structured_response(text: str) -> dict:
    """v28: 解析 PROMPT_FIGURE_ANALYZE 返回的 JSON

    容忍 markdown code fence (```json ... ```)
    容忍 LLM 在 JSON 前后输出额外解释文字（提取首个 JSON 块）
    """
    if not text:
        return _default_figure_structured()

    s = text.strip()

    # 1. 提取 markdown code block 内的 JSON
    ...
    if not s.startswith("{"):
        ...
        if brace_match:
            s = brace_match.group(0)

    # 3. 解析 JSON（容忍末尾逗号、单引号等小毛病）
    parsed = None
    try:
        import json
        parsed = json.loads(s)
        ...
        try:
            import json
            cleaned = re.sub(r",\s*}", "}", re.sub(r",\s*]", "]", s))
            parsed = json.loads(cleaned)
        except Exception as e:
            logger.warning(f"_parse_figure_structured_response JSON 解析失败: {e}, raw={text[:200]}")
            return _default_figure_structured()

    # 4. 合并默认值
    result = _default_figure_structured()
...
def _default_figure_structured() -> dict:
    """v28: PROMPT_FIGURE_ANALYZE 返回默认值（解析失败时使用）"""
    return {
        "figureNo": None,
        "figureType": "figure",
        "semanticTitle": None,
