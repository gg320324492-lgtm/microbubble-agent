...
# ============================================================================
# Component E: 段级识别成功后累积到 member.voice_embedding
# ============================================================================
def identify_and_learn_segments(
    seg_embs: list,
    labels: list,
    db,
    cluster_to_member: dict,
    conf_threshold: float = 0.7,
    max_segments_per_member: int = 10,
) -> dict:
    """每个段调 identify_speaker_by_embedding, 成功的累积到 member.voice_embedding

    解决问题: 韩重阳/张宏魁 enrolled 库只有 1 个 sample (samples=1),
    导致 center_dist=1.006 永远匹配不上. 会议里他们的发言如果能识别成功
    (即使 cluster 级 vote 失败), 把段 embedding 累积进 voice_embedding,
    下次会议该成员识别率显著提升.

    Args:
        seg_embs: 段 embedding 列表 (与 transcript 段一一对应)
        labels: KMeans labels (与 seg_embs 等长)
        db: AsyncSession
        cluster_to_member: {cluster_id: member_id} (从 vote_with_quality_gates 结果推)
        conf_threshold: 单段识别 conf 阈值
        max_segments_per_member: 每个 member 最多累积多少段 (防单会议过拟合)

    Returns:
        {member_id: learned_count}
    """
    ...
    # 1. 按 cluster 分组段 embedding
    cluster_segs = {}  # cid → [(seg_idx, seg_emb), ...]
    for i, (emb, lab) in enumerate(zip(seg_embs, labels)):
        if emb is None or np.all(np.array(emb) == 0) or lab < 0:
            continue
        cluster_segs.setdefault(lab, []).append((i, emb))

    # 2. 对每个 cluster 内每段调 identify_speaker_by_embedding
    # 收集: member_id → [seg_emb, ...] (成功的段)
    learn_per_member = {}  # member_id → list of seg_emb

    for cid, seg_list in cluster_segs.items():
        member_id = cluster_to_member.get(cid)
        if not member_id:
            continue
        # 该 cluster 已确定属于哪个 member, 但每段仍要 verify conf
        # (单段 conf > threshold 才累积, 防止中心很近但段很远的污染)
        for seg_idx, seg_emb in seg_list:
            try:
                # 这里用 sync 路径, 因为 identify_speaker_by_embedding 是 async
                # 但 seg_emb 已经是 numpy 数组, 直接调
                name, mid, conf = asyncio.get_event_loop().run_until_complete(
                    voiceprint_service.identify_speaker_by_embedding(db, np.array(seg_emb, dtype=np.float32))
                )
            except Exception:
                # event loop already running → 用 sync 路径
                name, mid, conf = _identify_sync(voiceprint_service, db, seg_emb)

            if mid == member_id and conf > conf_threshold:
                learn_per_member.setdefault(mid, []).append(seg_emb)

    # 3. 累积到 member.voice_embedding
    learned = {}
    for mid, embs in learn_per_member.items():
        embs = embs[:max_segments_per_member]
        if not embs:
            continue
        result = db.execute(select(Member).where(Member.id == mid))
        member = result.scalar_one_or_none()
        if not member:
            continue

        new_sample = np.mean(embs, axis=0).astype(np.float32)
        if member.voice_embedding is not None and (member.voice_sample_count or 0) > 0:
            old = np.array(member.voice_embedding, dtype=np.float32)
            weight = member.voice_sample_count / (member.voice_sample_count + len(embs))
            new_emb = old * weight + new_sample * (1 - weight)
        else:
            new_emb = new_sample

        member.voice_embedding = new_emb.tolist()
        member.voice_sample_count = (member.voice_sample_count or 0) + len(embs)
        learned[mid] = len(embs)
        logger.info(
            f"[learn] member_id={mid} ({member.name}) 累积 {len(embs)} 段 → "
            f"total samples={member.voice_sample_count}"
        )

    return learned


def _identify_sync(vp_service, db, seg_emb) -> tuple:
    """sync 版 identify_speaker_by_embedding (用于已运行 event loop 上下文)"""
    import asyncio
    from app.models.member import Member
    from sqlalchemy import select
    from app.services.voiceprint_service import MATCH_THRESHOLD, EMBEDDING_DIM

    if np.all(seg_emb == 0):
        return None, None, 0.0

    # 直接用 cosine_distance 计算
    emb_list = seg_emb.tolist()
    result = db.execute(
    ...
    for m in members:
        if m.voice_embedding is None:
            continue
        db_emb = np.array(m.voice_embedding, dtype=np.float32)
        if np.all(db_emb == 0):
            continue
        from app.services.voiceprint_voting import _cosine_distance
        dist = _cosine_distance(seg_emb, db_emb)
        if dist < best_dist:
            best_dist = dist
            best_name = m.name
            best_mid = m.id
...
# ============================================================================
# Component F: 用户验证段累积到 member.voice_embedding (2026-06-26 新增)
# ============================================================================
async def learn_from_verified_segments(
    member_id_to_segments: dict,
    db,
    max_segments_per_member: int = 15,
) -> dict:
    """用户验证的段 (cluster → name 映射) 累积到对应 member.voice_embedding

    解决问题 (2026-06-26 会议 #135):
      韩重阳/张宏魁 enrolled 只有 1 个 sample, embedding 跟实际声纹距离 0.7-1.0,
      永远匹配不上. 通过"用户手动 reprocess + override" 把 verified 段累积进
      voice_embedding, 下次会议该成员识别率显著提升.

    Args:
        member_id_to_segments: {member_id: [seg_emb_array, ...]}
        db: AsyncSession
        max_segments_per_member: 每个 member 最多累积多少段 (防单会议过拟合)

    Returns: {member_id: learned_count}
    """
    from app.models.member import Member
    from sqlalchemy import select

    learned = {}
    for mid, embs in member_id_to_segments.items():
        if not embs:
            continue
        embs = embs[:max_segments_per_member]
        ...
        new_sample = np.mean(embs, axis=0).astype(np.float32)
        old_sc = member.voice_sample_count or 0
        if member.voice_embedding is not None and old_sc > 0:
            old = np.array(member.voice_embedding, dtype=np.float32)
            # 加权平均: 已有 samples 权重更高 (避免单次会议过拟合)
            weight = old_sc / (old_sc + len(embs))
            new_emb = old * weight + new_sample * (1 - weight)
        else:
            new_emb = new_sample

        member.voice_embedding = new_emb.tolist()
        member.voice_sample_count = old_sc + len(embs)
        learned[mid] = len(embs)
        logger.info(
            f"[learn_verified] member_id={mid} ({member.name}) 累积 {len(embs)} 段 → "
            f"total samples={member.voice_sample_count}"
        )
