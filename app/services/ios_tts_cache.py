"""TTS pre-synthesize 缓存层 — iOS Safari 变体 (W77 第 1 批 B-1, W78 B-1 改名)

W78 B-1 类 20.12.1 修复: 原名 tts_cache.py 与 W77 B-2 (Android) 同名模块在
main merge 时互相覆盖 (B-2 后 merge 胜出), 导致 ios_tts_mainplay.py 的
`from app.services.tts_cache import TTSCacheStore` ImportError, 3 个 B-1
mainplay e2e 由 "20/20 PASS" 实际退化为 FAILED. 改名 ios_tts_cache.py 消歧,
Android 侧 tts_cache.py 保持原样不动 (渐进式, 不破坏 B-2 20/20)。
