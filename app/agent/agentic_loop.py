...
# 概念问 3 域 → 3 tool 硬下限 (#042 - 2026-06-28 chat agent 架构级集成; 2026-09-13 公式域随公式计算功能移除)
# 对齐 prompts.py _CROSS_DOMAIN_SYNTHESIS_SECTION 章节顺序 (知识 → 假设 → 成员)
# 与 #086 prompt 软规则协同: prompt 让 LLM 写"3 域综合", 代码保证 context 全
...
def _expand_concept_to_four_domain(planned: list[str]) -> list[str]:
    """explain_concept 域代码强制 fan-out (#042; 2026-09-13 公式域移除后为 3 域, 函数名保留历史)

    规则:
      1. 保留 planned 原顺序 + 原 tool (不删 LLM 已 planned 的, 包括非域 tool)
      2. 追加缺失的域 tool, 按 CONCEPT_DOMAIN_TOOLS 顺序补
      3. P2-3 fix (2026-07-08): 截断时**优先保留域 tool**, 非域被砍.
         之前实现: 简单 slice [:MAX], 当 LLM planned 6 个工具时按原顺序砍第 6 个,
         可能砍掉 query_members (域工具) 而保留 LLM 最后选的 get_meeting_transcript.
         修复: 域工具永远在结果前部, 截断时优先保留.
      4. 返回新 list (不修改原参数)

    示例:
      planned=['search_knowledge']
        → 域前移: [search_k, list_h, query_m] = 3
      planned=['search_knowledge', 'get_meeting_transcript']
        → 域前移: [list_h, query_m, search_k, get_meeting_transcript] = 4 (get_meeting 在尾)
      planned=['a', 'b', 'c', 'd', 'e', 'f'] (6 个非域)
        → 域前移: [search_k, list_h, query_m, a] = 4 (b/c/d/e/f 砍, 域全保)

    不变量:
      - len(result) ≤ AGENT_PLAN_STEP_MAX
      - **域 tool 全部保留** (除非 LLM 已 planned 5+ 个域 tool)
      - 原 planned 中非域 tool 优先被砍 (域优先)
    """
    ...
    # P2-3 fix: 把域工具移到前部, LLM planned 的非域保留在尾部 (按 LLM 顺序).
    # 截断 [:MAX] 时优先保留前部 (域), 尾部非域被砍.
...
class AgenticLoop:
    ...
    async def run(
        ...
        async def _run_inner() -> AsyncIterator[StreamEvent]:
            ...
            async for evt in self._run_legacy(messages, system, intent, ctx, max_rounds,
                                             llm, t0=t0):
                if evt.type == "done":
    ...
    async def _run_legacy(
        ...
        try:
            ...
            if (tool_loop_failed or not tool_calls or (_needs_write and not _write_done)) \
                ...
                # 2026-09-09 措辞修正 (实测 R2 事故): tool_loop_failed=False 且 0 调用时,
                # 老话术"工具调用暂时失败"与实际不符 — 模型还会从 SSE 渲染学这句,
                # 零调用也谎称失败。区分三种退化态 (第二轮实测 R4 补充"部分成功"态):
                ...
                elif tool_loop_failed:
                    _honest = "如实告知用户: '工具调用暂时失败，暂时无法查询到实时数据，请稍后重试'"
                else:
                    _honest = (
                ...
                guard = (
                    ...
                    f"{_state}\n"
                    "1. 严禁声称'已调用 query_meetings / query_projects 等工具'或'已获取相关信息'（指上下文里不存在的调用）\n"
                    "2. 严禁编造任何会议标题、日期、地点、任务、人名、数字列表\n"
                    f"3. {_honest}\n"
                    "4. 可以基于通用知识回答概念性问题, 但涉及系统内数据的部分必须明确标'无法核实'\n"
                    "5. 严禁引用'长期记忆'里关于会议/任务的具体内容来替代查询 — 未经验证的记忆引用必须明说来源是记忆而非本次查询\n"
                )
                system = system + guard
                logger.warning(
...
def _extract_prev_round_entities(messages: list[dict]) -> tuple[list[str], list[str]]:
    ...
    2026-09-10 e2e 实测纪律:
    - 实体源优先级: tool_trace 里 tool_use.input 的实体 (模型上轮自己解析过的
      权威值, 上轮查了 assignee_name=韩重阳 → "他"几乎必指他) > "负责人：X"
      角色标注 > 文本首现人名。
    - 不能只看最后一条 assistant — R2 答案常是"是否需要查看他…"的元话术轮,
      实体在前面的实质回答里。
    - 更早轮大列表首名≠本轮主语 → 文本兜底只信最近一条 (fail-closed 宁缺勿错)。
    - 收敛为单锚点: 注入多候选 = 没锚 (模型仍挑错)。
    """
    roster = _member_names_cache()
    _title_stop = ("进行中任务", "已完成任务", "待办任务", "任务列表", "无", "暂无")
    _person_keys = ("assignee_name", "member_name", "name")
    _title_keys = ("title_keyword", "keyword", "project_name", "query")
    tool_names: list[str] = []
    ...
    names = [n for n in dict.fromkeys(tool_names + labeled + found_sorted) if n in roster]
    out_titles = list(dict.fromkeys(tool_titles + titles))
    return names[:1], out_titles[:2]
