...
关键区分点（容易混淆）：
- 「刚才 X」/「之前说的 X」/「你提到的 X」→ **优先 follow_up 或按上文实体归类**
  (2026-09-10 实测: "刚才那条备注写上了吗"被判 search_info 置信 60 → 模型丢开上文
  实况重新泛答, 否定了本会话里刚执行过的写操作。上下文 messages 里有该操作记录,
  分类为 follow_up/data_query 并带上上文实体即可直答)
